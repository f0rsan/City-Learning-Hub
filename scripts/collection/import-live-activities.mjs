...
const reportsDir = join(process.cwd(), "docs", "superpowers", "reports");
const expectedReportPath = join(reportsDir, `live-source-calibration-${reportDate}.json`);
...
function latestReportPath() {
  return readdirSync(reportsDir)
    .filter((fileName) => /^live-source-calibration-\d{4}-\d{2}-\d{2}\.json$/.test(fileName))
    .sort()
    .at(-1);
}
...
const latestReport = latestReportPath();
const reportPath = existsSync(expectedReportPath)
  ? expectedReportPath
  : latestReport
    ? join(reportsDir, latestReport)
    : expectedReportPath;
...
function shenzhenDate(year, monthIndex, day, hour = 10, minute = 0) {
  return new Date(Date.UTC(year, monthIndex, day, hour - 8, minute, 0, 0));
}

function shenzhenCalendarParts(date) {
  const shifted = new Date(date.getTime() + shenzhenOffsetMs);
  return {
...
function addDays(date, days) {
  const { year, monthIndex, day } = shenzhenCalendarParts(date);
  return shenzhenDate(year, monthIndex, day + days);
}
...
function parseDateFromTitle(title) {
  ...
  if (chineseDate) {
    return shenzhenDate(currentYear, Number(chineseDate[1]) - 1, Number(chineseDate[2]));
  }

  const dotDate = normalized.match(/(?:^|[^\d])(\d{1,2})[./](\d{1,2})(?=深圳站|[^\d]|$)/);
  if (dotDate) {
    return shenzhenDate(currentYear, Number(dotDate[1]) - 1, Number(dotDate[2]));
  }
...
function toActivity(source, item, globalIndex) {
  ...
  const canListPublicly = !expired && isTrusted && relevant && !sourceOnlyFallback;
  const publicListingTier = canListPublicly
    ? dateIsExplicit && isDirectAuto
      ? "featured"
      : "reference"
    : undefined;
...
// Source report: ${relative(process.cwd(), reportPath)}
